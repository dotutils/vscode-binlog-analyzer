...
import { showComparisonWebview } from './comparisonWebview';
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('binlog.compareTimelines', async () => {
            ...
            await showComparisonWebview(context, mcpClient, allBinlogPaths[0], allBinlogPaths[1]);
        })
    );
...
async function addBinlogs(newPaths: string[]) {
    ...
    // Restart the MCP client with the updated binlog paths and wait for it
    try {
    ...
    // If we now have 2+ binlogs, automatically open the comparison view
    if (allBinlogPaths.length >= 2 && extensionContext && mcpClient) {
        await showComparisonWebview(extensionContext, mcpClient, allBinlogPaths[0], allBinlogPaths[1]);
    }
}
